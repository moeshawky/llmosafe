...
class TestCalculateHalo:
    """Tests for calculate_halo function — dual-path sifter (classifier + keyword bias)."""

    def test_empty_string(self) -> None:
        """Empty string returns low entropy (classifier intercept only, no tokens)."""
        result = llmosafe.calculate_halo("")
        assert 0 <= result <= 16000  # classifier intercept, no tokens, no keywords

    def test_no_bias(self) -> None:
        """Normal text should have low bias."""
        result = llmosafe.calculate_halo("This is a normal sentence.")
        assert 0 <= result <= 20000

    def test_authority_bias(self) -> None:
        """Authority keywords should increase bias score."""
        result = llmosafe.calculate_halo("The expert recommendation")
        assert result > 0

    def test_multiple_bias_types(self) -> None:
        """Multiple bias keywords should accumulate."""
        result = llmosafe.calculate_halo("The expert official certified recommendation")
        assert result >= 100
    ...
    def test_case_insensitive(self) -> None:
        ...
        # Case-insensitive: mixed-case, lower, and upper all match keywords.
        # All-caps may add extra emphasis signal.
        assert lower == upper  # both match "expert" keyword, same classifier output
        assert lower > 0

    def test_negation_awareness(self) -> None:
        """'not an expert' should produce lower authority score than 'expert' alone."""
        negated = llmosafe.calculate_halo("This is not an expert opinion")
        positive = llmosafe.calculate_halo("This is an expert opinion")
        # Negation reduces signal — negative sentence has lower or equal entropy
        assert negated <= positive

    def test_all_8_categories(self) -> None:
...
class TestCheckResources:
    ...
    def test_reasonable_ceiling(self) -> None:
    ...
    def test_small_ceiling(self) -> None:
    ...
    def test_monitors_rss_not_disk(self) -> None:
...
class TestMakeSynapse:
    ...
    def test_entropy_only(self) -> None:
    ...
    def test_entropy_masked(self) -> None:
    ...
    def test_stability_integration(self) -> None:
        """make_synapse produces values compatible with get_stability.
        v0.7.0 thresholds: 0-40000 stable, 40000-50000 pressure, >50000 unstable.
        """
        assert llmosafe.get_stability(make_synapse(400)) == 0
        assert llmosafe.get_stability(make_synapse(51000)) == -2
        assert llmosafe.get_stability(make_synapse(400, has_bias=True)) == -3

    def test_process_synapse_integration(self) -> None:
...
class TestGetStability:
    ...
    def test_zero_synapse(self) -> None:
        """Zero synapse should be stable."""
        assert llmosafe.get_stability(0) == 0

    def test_pressure_boundary(self) -> None:
        """Entropy at 40000 is stable, 40001 triggers pressure (CognitiveInstability).
        v0.7.0: PRESSURE_THRESHOLD = 40000 (strict less-than).
        """
        assert llmosafe.get_stability(make_synapse(40000)) == 0
        assert llmosafe.get_stability(make_synapse(40001)) == -2
    ...
    def test_return_code_values(self) -> None:
        ...
        codes_seen.add(llmosafe.get_stability(make_synapse(51000)))  # -2
...
class TestProcessSynapse:
    ...
    def test_valid_synapse(self) -> None:
    ...
    def test_unstable_synapse(self) -> None:
        ...
        v0.7.0: STABILITY_THRESHOLD = 50000.
        """
        result = llmosafe.process_synapse(make_synapse(51000))
        assert result == -2

    def test_high_surprise(self) -> None:
        """Surprise > 58000 should return -4 (HallucinationDetected).
        v0.7.0: HallucinationDetected threshold = 58000.
        """
        result = llmosafe.process_synapse(make_synapse(400, surprise=58001))
        assert result == -4
...
class TestSystemMetrics:
    ...
    def test_entropy_is_predictive_not_enforcement(self) -> None:
...
class TestExceptions:
    ...
    def test_resource_exhausted_direct(self) -> None:
...
class TestDiskGuardComposition:
    ...
    def test_check_resources_before_write(self) -> None:
    ...
    def test_entropy_as_early_warning(self) -> None:
