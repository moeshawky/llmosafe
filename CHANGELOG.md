...
- **Fixed:** Zero cooldown on `decide_with_pressure()` pressure-override path (was `0ms`, now `5000ms`) — this early-return path bypasses `decide()` entirely and was missed in the v0.5.5 cooldown fix, causing immediate re-entry into the safety loop when pressure escalation occurred
