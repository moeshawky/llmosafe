...
mod c_abi {
    ...
    pub extern "C" fn llmosafe_calculate_halo(
        ...
        let slice = unsafe { core::slice::from_raw_parts(text_ptr as *const u8, text_len) };
...
mod tests {
    ...
    fn test_c_abi_invalid_utf8() {
        ...
        let result = crate::c_abi::llmosafe_calculate_halo(
            ...
            invalid_data.len() - 1, // Exclude null terminator
        );
        let _ = result;
    }
    ...
    fn test_c_abi_very_long_string() {
        let long_string = std::vec![b'a'; 1024 * 1024];
        let result = crate::c_abi::llmosafe_calculate_halo(
