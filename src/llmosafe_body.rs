...
impl EnvironmentalVitals {
    ...
    fn read_iowait() -> Option<u64> {
        // Opt: Avoid BufReader allocation for single-line read by using read_to_string
...
mod tests {
    ...
    fn test_check_ctrl_valid_ceiling_returns_body_output() {
        let guard = ResourceGuard::for_testing(1024 * 1024 * 1024 * 1024, 100, 20); // Set high enough to not hit ratio >= 1.0 on test machines
