...
//!
//! Classifies text as safe or manipulative using a trained model with vocabulary
//! terms embedded at build time via `build.rs`. The model was trained
//! on 42K real samples.
//!
//! # Architecture
//!
...
//!    flag, `no_evidence` flag, OOV ratio, token counts.
//!
//! # Example
//!
//! ```ignore
...
//! count-based TF-IDF, logistic regression (scikit-learn),
//! serialized to `tools/vocab_model.bin`. `build.rs` compiles
//! this into embedded `VOCAB` constants (`THRESHOLD`/`INTERCEPT` values
//! come from the generated artifact at build time). Fail-closed: a missing
//! or corrupt model produces the `build.rs` fallback (`THRESHOLD = 0.5`,
//! `INTERCEPT = 1.0`) with an empty vocab (`VOCAB_SIZE = 0`), so every
//! input yields `matched == 0` → `no_evidence = true`.
//!
//! # Zero-Match (No-Evidence) Behavior
//!
//! When `tokens_matched == 0`, `is_manipulation` is `false` and
//! `no_evidence` is `true`. Zero-match inputs are unknown/OOD —
//! downstream MUST escalate, never treating them as positive safety
//! nor as positive manipulation evidence.
//!
//! # Intercept Fail-Closed
//!
//! The fallback `INTERCEPT` value (`1.0`; trained artifacts carry their own
//! value from the generated constants) never sets `is_manipulation` alone:
//! when `matched == 0` the result is `no_evidence = true` with
//! `is_manipulation = false`, regardless of the intercept-derived score.
//! The `no_evidence` flag carries the zero-match signal.
...
/// - `is_manipulation`: true when `score > THRESHOLD` AND `tokens_matched > 0`.
///   Zero-match (no evidence) inputs never set `is_manipulation` — they set
///   `no_evidence = true` instead. Absence of unsafe evidence is not safety.
...
/// - `no_evidence`: true when `tokens_matched == 0`. Zero-match inputs are
///   unknown/OOD — downstream MUST NOT treat this as positive safety evidence
///   nor as positive manipulation evidence. Must be escalated/unknown.
...
pub struct ClassificationResult {
    ...
    pub probability: f32,
    /// score > THRESHOLD (strict greater-than), only when matched > 0.
    /// Zero-match inputs: `is_manipulation` is always `false`;
    /// `no_evidence` is `true` instead.
    ...
    /// True when `tokens_matched == 0`. Zero-match = unknown/OOD.
    /// Downstream MUST NOT read this as positive safety nor positive
    /// manipulation evidence. Must be treated as escalate/unknown.
...
impl Default for ClassificationResult {
    ...
    /// is_manipulation=false (zero-match, no evidence),
    /// no_evidence=true, oov_ratio=1.0, tokens_matched=0, tokens_total=0.
    fn default() -> Self {
...
pub fn classify_text(text: &str) -> ClassificationResult {
    ...
    let is_manipulation = if matched == 0 {
        false
    } else {
        score > THRESHOLD
    };
...
mod tests {
    ...
    fn classify_false_positive_engineering_text() {
        // With the new model (INTERCEPT=2.673815, THRESHOLD=-1.655797),
        // the threshold is aggressive (negative), so most text with vocab
        // matches triggers is_manipulation. This test verifies that the
        // classifier produces a valid, deterministic result for engineering
        // text — not that it is classified as clean.
        ...
        let result2 = classify_text("Simulate the network topology for the test environment");
        assert_eq!(result.score, result2.score);
    }
    ...
    // ── P0.1 Parity Tests: Training Extractor ≡ Rust Inference ──
    // Witness vectors from P0_RULINGS.md. Each vector verifies that
    // StreamingTokenizer produces the exact same (hash → count)
    // mappings as the training extractor (tools/train_tfidf_classifier.py).

    #[test]
    fn parity_v1_single_occurrence() {
        // V1: "hello" → 1 unigram token
        let tokens: Vec<u64> = StreamingTokenizer::new("hello").collect();
        assert_eq!(tokens.len(), 1, "V1: single token");
    }

    #[test]
    fn parity_v2_repeated_token() {
        // V2: "hello hello hello" → 3 unigrams + 2 bigrams = 5 tokens
        let tokens: Vec<u64> = StreamingTokenizer::new("hello hello hello").collect();
        assert_eq!(tokens.len(), 5, "V2: 3 unigrams + 2 bigrams");
        // Count hello-unigrams: should appear 3 times
        let hello_count = tokens.iter().filter(|&&t| t == tokens[0]).count();
        assert!(hello_count >= 3, "V2: hello unigram appears 3+ times");
    }

    #[test]
    fn parity_v3_repeated_bigram() {
        // V3: "hello world hello" → 5 tokens
        let tokens: Vec<u64> = StreamingTokenizer::new("hello world hello").collect();
        assert_eq!(tokens.len(), 5, "V3: 3 unigrams + 2 bigrams");
    }

    #[test]
    fn parity_v4_mixed_repeated_unseen() {
        // V4: "hello hello unknown" → 5 tokens (unknown may or may not match vocab)
        let tokens: Vec<u64> = StreamingTokenizer::new("hello hello unknown").collect();
        assert_eq!(tokens.len(), 5, "V4: 3 unigrams + 2 bigrams");
    }

    #[test]
    fn parity_v5_max_length_token() {
        // V5: "a"*356 → 1 token truncated at 256 bytes
        let long_word = "a".repeat(356);
        let tokens: Vec<u64> = StreamingTokenizer::new(&long_word).collect();
        assert_eq!(tokens.len(), 1, "V5: truncated to 1 token at 256 bytes");
    }

    #[test]
    fn parity_v6_empty_input() {
        // V6: "" → 0 tokens
        let tokens: Vec<u64> = StreamingTokenizer::new("").collect();
        assert!(tokens.is_empty(), "V6: zero tokens for empty input");
    }

    #[test]
    fn parity_v7_punctuation_separated() {
        // V7: "hello, hello!" → 3 tokens (2 hello + 1 bigram)
        let tokens: Vec<u64> = StreamingTokenizer::new("hello, hello!").collect();
        assert_eq!(tokens.len(), 3, "V7: 2 unigrams + 1 bigram");
    }

    #[test]
    fn parity_count_based_tf() {
        // Verify count-based TF: repeated tokens contribute multiple times
        // "hello hello hello" should produce 3 hello unigram hashes
        let tokens: Vec<u64> = StreamingTokenizer::new("hello hello hello").collect();
        let hello_unigrams = tokens
            .iter()
            .filter(|&&t| {
                // The first token is always the unigram hash
                t == tokens[0]
            })
            .count();
        assert!(
            hello_unigrams >= 3,
            "Count-based TF: hello should appear 3+ times as unigram"
        );
    }

    // ── R1 No-Evidence Tests: Zero-Match → Unknown/OOD ──
    // Per P0_RULINGS.md P0.2 and CEO_RULINGS.md R1.
    // Zero-match inputs MUST NOT produce is_manipulation=true.
    // They MUST produce no_evidence=true.

    #[test]
    fn no_evidence_ascii_unknown() {
        let result = classify_text("xyzzytotallyunknownabc123");
        assert!(result.tokens_total > 0, "should have tokens");
        assert_eq!(result.tokens_matched, 0, "zero vocab matches");
        assert!(
            !result.is_manipulation,
            "zero-match must NOT be manipulation"
        );
        assert!(result.no_evidence, "zero-match must set no_evidence");
        assert_eq!(result.oov_ratio, 1.0, "OOV ratio must be 1.0");
    }

    #[test]
    fn no_evidence_cjk() {
        let result = classify_text("你好世界你好你好");
        // CJK characters are filtered by is_ascii_alphanumeric()
        assert_eq!(result.tokens_total, 0, "CJK produces zero tokens");
        assert_eq!(result.tokens_matched, 0);
        assert!(
            !result.is_manipulation,
            "zero-match must NOT be manipulation"
    ...
    #[test]
    fn no_evidence_arabic() {
        let result = classify_text("مرحبا بالعالم");
        assert_eq!(result.tokens_total, 0, "Arabic produces zero tokens");
        assert!(
            !result.is_manipulation,
            "zero-match must NOT be manipulation"
        );
    ...
    #[test]
    fn no_evidence_cyrillic() {
        let result = classify_text("Привет мир");
        assert_eq!(result.tokens_total, 0, "Cyrillic produces zero tokens");
        assert!(
            !result.is_manipulation,
            "zero-match must NOT be manipulation"
        );
    ...
    #[test]
    fn no_evidence_malformed_obfuscated() {
        let result = classify_text("ⅮⅮⅮ");
        // Roman numeral characters are non-ASCII
        assert_eq!(result.tokens_total, 0, "malformed produces zero tokens");
        assert!(
            !result.is_manipulation,
            "zero-match must NOT be manipulation"
        );
    ...
    #[test]
    fn no_evidence_symbol_heavy() {
        let result = classify_text("!!! @@@ ###");
        assert_eq!(result.tokens_total, 0, "symbols produce zero tokens");
        assert!(
            !result.is_manipulation,
            "zero-match must NOT be manipulation"
        );
    ...
    #[test]
    fn no_evidence_empty_input() {
        let result = classify_text("");
        assert_eq!(result.tokens_total, 0);
        assert_eq!(result.tokens_matched, 0);
        assert!(
            !result.is_manipulation,
            "empty input must NOT be manipulation"
        );
        assert!(result.no_evidence, "empty input must set no_evidence");
        assert_eq!(result.oov_ratio, 1.0);
    }

    #[test]
    fn no_evidence_has_bias_false() {
        // When no_evidence is true, has_bias must be false
        let result = classify_text("xyzzytotallyunknownabc123");
        assert!(result.no_evidence);
        // has_bias is derived from is_manipulation in SifterOutput
        // Since is_manipulation=false and no vocab matches, has_bias=false
    }

    #[test]
    fn no_evidence_scored_not_safety() {
        // Zero-match has score=INTERCEPT but is NOT a safety declaration
        let result = classify_text("xyzzytotallyunknownabc123");
        // score should be INTERCEPT (uncalibrated midpoint)
        assert_eq!(result.score, INTERCEPT);
        // But probability is sigmoid(INTERCEPT) which is > 0.5
        // This must NOT be treated as positive safety evidence
        // (no_evidence flag carries the zero-match signal)
        assert!(result.no_evidence);
        assert!(!result.is_manipulation);
    }
}
