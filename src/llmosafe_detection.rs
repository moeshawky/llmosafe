...
impl RepetitionDetector {
    ...
    pub fn unique_patterns(&self) -> usize {
        ...
        for &hash in self.history.iter() {
            // Optimization: Use slice .contains() instead of manual iteration with .iter().take(count) to leverage direct CPU instructions and avoid iterator overhead.
            if !seen[..count].contains(&hash) && count < MAX_CONTEXT_LEN {
                seen[count] = hash;
                count += 1;
            }
        }
